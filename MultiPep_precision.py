...
n = ['val_0.5_mean_precision', 'val_0.7_mean_precision', 'val_0.9_mean_precision',
'val_0.5_vs_0.7_precision_pval', 'val_0.5_vs_0.9_precision_pval',
'val_0.5_vs_0.7_precision_qval', 'val_0.5_vs_0.9_precision_qval']
n += [i.replace("precision", "fps") for i in n]
...
for i in range(10):
    ...
    for jj,ii in enumerate(files):
        ...
        for iii in th:
            ...
            fp_count_total = np.sum((1 - ys) * tmp_predictions, axis=0)
            TP = np.sum(ys * tmp_predictions, axis=0)
            
            print("\t\tmean prediction score:", np.mean(TP / (TP+ fp_count_total)))
            print("\t\tmean TPs:", np.mean(TP))
            
            prec_dat["{}_{}_mean_fps".format(typ, iii)].append(np.mean(fp_count_total))
            
            # getting precision score for every class
            for iiii in range(len(nms)):
                pr = precision_score(ys[:,iiii],  tmp_predictions[:,iiii])
                tmp_prec.append(pr)
            
            
            prec_dat["{}_{}_mean_precision".format(typ, iii)].append(np.mean(tmp_prec))
            ...
            if iii > 0.5:
                prec_dat["{}_{}_vs_{}_precision_pval".format(typ, 0.5, iii)].append(wilcoxon(pr05, tmp_prec)[1])
                prec_dat["{}_{}_vs_{}_fps_pval".format(typ, 0.5, iii)].append(wilcoxon(fp05, fp_count_total)[1])
        
pval_names = []
fps_count_names = []
for k,v in prec_dat.items():
    if "_pval" in k:
        pval_names.append(k)
        
for i in pval_names:
    print(i)
    prec_dat[i.replace("pval", "qval")] = multi.fdrcorrection(prec_dat[i], alpha=0.01, method = 'indep')[1]




df = pd.DataFrame(prec_dat)
df = df[n]
df.index = ["model_{}".format(i) for i in range(10)]
df.to_excel("precision_data.xlsx")
