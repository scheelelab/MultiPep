...
parser.add_argument("-pred_type", type=str, choices=["mean","max","vote"], default="mean", required=False)
...
if args.pred_type == "vote":
    print("using 'vote' as pred_type")
    
    avr = [np.round(i) for i in alprds[0]]
    #for i in alprds[0]:
    #    avr.append(i)
    
    for i in alprds[1:]:
        for ii in range(len(i)):
            avr[ii] += np.round(i[ii])
